...
def store_hits(fname = "../ntuples/hgcalNtuple_ele15_n100_testhelper.root"):
    ...
    for event in array:
        ...
        # with selection
        sel_hit_indices = (event['tc_energy'] > -0.01)# & (event['tc_layer'] < 29.)
        #sel_hit_indices = (event['tc_energy'] > -0.01) & (event['tc_z'] > 0.) & (event['tc_layer'] < 29.)
        #sel_hit_indices = (event['tc_energy'] > -0.01) & (event['tc_z'] > 0.) & (event['tc_layer'] < 39.) & (event['tc_flags'] < 3)
        #sel_hit_indices = (event['tc_energy'] > 0.01) & (event['tc_layer'] < 39.) & (event['tc_flags'] < 3)


        print "Found %i hits" % len(sel_hit_indices)
        if len(sel_hit_indices) < 20: continue
        ...
        if len(core_samples_mask) < 10: continue
        ...
        for k, col in zip(unique_labels, colors):
            ...
            if len(xyz) > 1:
                ...
                if energy > -1:
                    ...
                    title += "PCA: %f" % pca.explained_variance_ratio_[0]
                    plt.title(title)
